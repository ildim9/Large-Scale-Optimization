...
class Solver:
    ...
    def VND(self):
        ...
        while k <= kmax:
            self.InitializeOperators(rm, sm) 
            if k == 1:
                self.FindBestRelocationMove(rm)     # Implementation of Relocation move
                ...
                self.FindBestSwapMove(sm)           # Implementation of Swap move
            ...
            if candidateCust.isRouted is False:
                if rt.cost + candidateCust.service_time <= rt.capacity:
                    for j in range(0, len(rt.sequenceOfNodes) - 1):
                        ...
                        trialCost = costAdded - costRemoved
                        trialprof = candidateCust.profit
                        knapsack = (-(10 ** 12) * trialprof/costAdded)
                        if rt.cost + trialCost > rt.capacity:
                            continue
                        if knapsack < best_insertion.knaspsack:
